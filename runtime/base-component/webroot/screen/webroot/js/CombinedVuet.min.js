...
window.define||(window.define=function(a,b,c){moqui.isString(a)||(c=b,b=a);moqui.isArray(b)||(c=b);return moqui.isFunction(c)?c():c});moqui.localeMap={zh:"zh-cn"};moqui.objToSearch=function(a){var b="";moqui.isPlainObject(a)&&$.each(a,function(a,d){b=b+(0<b.length?"\x26":"")+a+"\x3d"+d});return b};
moqui.searchToObj=function(a){if(!a||0===a.length)return{};var b={};a=a.split("\x26");for(var c=0;c<a.length;c++){var d=a[c].split("\x3d");if(1<d.length){var e=d[0],d=d[1],f=b[e];f?moqui.isArray(f)?f.push(d):b[e]=[f,d]:b[e]=d}}return b};Vue.filter("decodeHtml",moqui.htmlDecode);
...
moqui.notifyOpts={delay:1500,timer:500,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"success",animate:{enter:"animated fadeInDown",exit:""}};moqui.notifyOptsInfo={delay:5E3,timer:500,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"info",animate:{enter:"animated fadeInDown",exit:""}};
moqui.notifyOptsError={delay:15E3,timer:500,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"danger",animate:{enter:"animated fadeInDown",exit:""}};
...
moqui.handleAjaxError=function(a,b,c){var d=a.responseText,e;try{e=JSON.parse(d)}catch(g){}console.warn("ajax "+b+" ("+a.status+"), message "+c);var f=!1;e&&moqui.isPlainObject(e)?f=moqui.notifyMessages(e.messageInfos,e.errors,e.validationErrors):d&&moqui.isString(d)&&d.length&&(f=moqui.notifyMessages(d));401===a.status?moqui.webrootVue?window.location.href=moqui.webrootVue.currentLinkUrl:window.location.reload(!0):0===a.status?0>c.indexOf("abort")&&($.notify(new moqui.NotifyOptions("Could not connect to server",
null,"danger",null),moqui.notifyOptsError),moqui.webrootVue.addNotify("Could not connect to server","danger")):f||(a="Error: "+c+" ("+b+")",$.notify(new moqui.NotifyOptions(a,null,"danger",null),moqui.notifyOptsError),moqui.webrootVue.addNotify(a,"danger"))};moqui.componentCache=new moqui.LruMap(50);moqui.handleLoadError=function(a,b,c){"abort"===b?console.warn("load aborted: "+b+" ("+a.status+"), message "+c):(moqui.webrootVue.loading=0,moqui.handleAjaxError(a,b,c))};
moqui.loadComponent=function(a,b,c){var d,e,f,g;if("string"===typeof a){var k=a.indexOf("?");0<k?(d=a.slice(0,k),f=a.slice(k+1)):d=a}else d=a.path,e=a.extraPath,f=a.search,g=a.bodyParameters;var h=d,m=".js"===d.slice(-3);!m&&a.renderModes&&0<=a.renderModes.indexOf("js")&&(h+=".js",m=!0);m||(h+=".vuet");e&&0<e.length&&(h+="/"+e);f&&0<f.length&&(h+="?"+f);console.info("loadComponent "+h+(c?" id "+c:""));a={type:"GET",url:h,error:moqui.handleLoadError,success:function(a,e,f){if(205===f.status)a=f.getResponseHeader("X-Redirect-To"),
moqui.webrootVue.setUrl(a);else{a||b(moqui.NotFound);var g=0<=f.getResponseHeader("Cache-Control").indexOf("max-age");if(moqui.isString(a)&&0<a.length)if(m||"define("===a.slice(0,7)){console.info("loaded JS from "+h+(c?" id "+c:""));var k=eval(a);k.template?(g&&moqui.componentCache.put(d,k),b(k)):(a=(".js"===d.slice(-3)?d.slice(0,-3):d)+".vuet",$.ajax({type:"GET",url:a,error:moqui.handleLoadError,success:function(a){k.template=a;g&&moqui.componentCache.put(d,k);b(k)}}))}else a=a.replace(/<script/g,
"\x3cm-script").replace(/<\/script>/g,"\x3c/m-script\x3e").replace(/<link/g,"\x3cm-stylesheet"),console.info("loaded HTML template from "+h+(c?" id "+c:"")),a={template:"\x3cdiv"+(c&&0<c.length?' id\x3d"'+c+'"':"")+"\x3e"+a+"\x3c/div\x3e"},g&&moqui.componentCache.put(d,a),b(a);else moqui.isPlainObject(a)?a.screenUrl&&0<a.screenUrl.length?moqui.webrootVue.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&window.location.replace(a.redirectUrl):b(moqui.NotFound)}}};g&&!$.isEmptyObject(g)&&(a.type=
"POST",a.data=g);return $.ajax(a)};moqui.NotFound=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3ch4\x3eScreen not found at {{this.$root.currentPath}}\x3c/h4\x3e\x3c/div\x3e'});moqui.EmptyComponent=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3e\x26nbsp;\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e'});
...
Vue.component("m-form",{props:{action:{type:String,required:!0},method:{type:String,"default":"POST"},submitMessage:String,submitReloadId:String,submitHideId:String,focusField:String,noValidate:Boolean},data:function(){return{fields:{},fieldsChanged:{},buttonClicked:null}},template:'\x3cform @submit.prevent\x3d"submitForm"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/form\x3e',methods:{submitForm:function(){var a=$(this.$el);if(this.noValidate||a.valid()){var b=null,c=null,d=$(this.buttonClicked||document.activeElement);
d.length&&a.has(d)&&d.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(d.is("[name]")&&(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=new FormData(this.$el);e.append("moquiSessionToken",this.$root.moquiSessionToken);$.each(this.fields,function(a,b){e.append(a,b)});b&&e.append(b,c);this.$root.loading++;$.ajax({type:this.method,url:this.$root.appRootPath+this.action,data:e,contentType:!1,processData:!1,headers:{Accept:"application/json"},
error:moqui.handleLoadError,success:this.handleResponse})}else if(!this.noValidate){var f=a.find("div.has-error input, div.has-error select, div.has-error textarea").first();f.length&&(a=f.parents("div.panel-collapse.collapse").last(),a.length?a.hasClass("in")?f.focus():(a=a.prevAll("div.panel-heading").last(),a.length?(b=a.find('a[data-toggle\x3d"collapse"]').first(),b.length?b.click():a.click(),setTimeout(function(){f.focus()},250)):f.focus()):f.focus())}},handleResponse:function(a){this.$root.loading--;
var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&$("#"+a).modal("hide");(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),$.notify(new moqui.NotifyOptions(b,
null,"success",null),moqui.notifyOpts),moqui.webrootVue.addNotify(b,"success")):b||$.notify(new moqui.NotifyOptions("Submit successful",null,"success",null),moqui.notifyOpts)},fieldChange:function(a){a=a.delegateTarget;var b=$(a);b.hasClass("input-group")&&b.children("input").length&&(b=b.children("input").first(),a=b.get(0));var c=!1;if("INPUT"===a.nodeName||"TEXTAREA"===a.nodeName)c="radio"===b.attr("type")||"checkbox"===b.attr("type")?a.checked!==a.defaultChecked:a.value!==a.defaultValue;else if("SELECT"===
a.nodeName)if(a.multiple)for(var d=a.options.length,e=0;e<d;e++){var f=a.options[e];if(f.selected!==f.defaultSelected){c=!0;break}}else c=!a.options[a.selectedIndex].defaultSelected;c?(this.fieldsChanged[b.attr("name")]=!0,b.parents(".form-group").children("label").addClass("is-changed"),b.parents(".form-group").find(".select2-selection").addClass("is-changed"),b.addClass("is-changed")):(this.fieldsChanged[b.attr("name")]=!1,b.parents(".form-group").children("label").removeClass("is-changed"),b.parents(".form-group").find(".select2-selection").removeClass("is-changed"),
b.removeClass("is-changed"))}},mounted:function(){var a=this,b=$(this.$el);this.noValidate||b.validate({errorClass:"help-block",errorElement:"span",highlight:function(a,b,e){$(a).parents(".form-group").removeClass("has-success").addClass("has-error")},unhighlight:function(a,b,e){$(a).parents(".form-group").removeClass("has-error").addClass("has-success")}});b.find('[data-toggle\x3d"tooltip"]').tooltip({placement:"auto top"});this.focusField&&0<this.focusField.length&&b.find('[name^\x3d"'+this.focusField+
'"]').addClass("default-focus").focus();b.find(":input").on("change",this.fieldChange);b.find("div.input-group.date").on("change",this.fieldChange);b.find('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]').on("click",function(){a.buttonClicked=this})}});
...
moqui.isArray(b)&&(e.navMenuList=b)}});window.history.pushState(null,this.ScreenTitle,a)}}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=this.currentPathList,b=this.activeSubscreens;console.info("reloadSubscreens currentPathList "+JSON.stringify(this.currentPathList));
if(0===a.length&&0<b.length)b.splice(1),b[0].loadActive();else for(var c=0;c<b.length&&!(c>=a.length);c++)b[c].loadActive()&&b.splice(c+1)},goPreviousScreen:function(){for(var a=this.currentPath,b=this.navHistoryList,c,d=0;d<b.length;d++)if(0>b[d].pathWithParams.indexOf(a)){c=b[d];break}c&&c.pathWithParams&&c.pathWithParams.length&&this.setUrl(c.pathWithParams)},addContainer:function(a,b){this.activeContainers[a]=b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+
a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(a){b.navPlugins.push(a)})},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=
c},switchDarkLight:function(){var a=$("body");a.toggleClass("bg-dark");a.toggleClass("bg-light");a=a.hasClass("bg-dark")?"bg-dark":"bg-light";$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"OUTER_STYLE",preferenceValue:a}})},showScreenDocDialog:function(a){$("#screen-document-dialog").modal("show");$("#screen-document-dialog-body").load(this.currentPath+"/screenDoc?docIndex\x3d"+a)},stopProp:function(a){a.stopPropagation()},
getNavHref:function(a){a||(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);0===a.indexOf(this.basePath)&&(a=a.replace(this.basePath,this.linkBasePath));return a}},watch:{navMenuList:function(a){if(0<
...
"moquiSessionToken"],f=c.substring(e+1).split("\x26"),c=c.substring(0,e),e=0,g="",k=0;k<f.length;k++){var h=f[k],c=-1===c.indexOf("?")?c+"?":c+"\x26",c=c+h;if(!(3<e)){var m=h.indexOf("\x3d");if(0<m){var l=h.substring(0,m),h=h.substring(m+1);0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")||0<d.indexOf(l)||l===h||(0<g.length&&(g+=", "),g+=decodeURIComponent(h),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===
...
this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},basePathSize:function(){return this.basePath.split("/").length-this.appRootPath.split("/").length},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-1].title:""}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();
this.userId=$("#confUserId").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);var a=this;$(".confNavPluginUrl").each(function(b,c){a.addNavPlugin($(c).val())});this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws")},mounted:function(){var a=$(this.$el);a.find('.navbar [data-toggle\x3d"tooltip"]').tooltip({placement:"bottom",trigger:"hover"});a.find("#history-menu-link").tooltip({placement:"bottom",
trigger:"hover"});a.find("#notify-history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#document-menu-link").tooltip({placement:"bottom",trigger:"hover"});this.setUrl(window.location.pathname+window.location.search);this.notificationClient.registerListener("ALL");$("#screen-document-dialog").on("hidden.bs.modal",function(){var a=$("#screen-document-dialog-body");a.empty();a.append('\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3eLoading�\x3c/div\x3e\x3c/div\x3e')});window.Notification&&
